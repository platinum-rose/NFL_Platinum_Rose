#!/usr/bin/env node
// Build the manually reviewed BetOnline 2026-08-10 futures import from BEO_*_0810 screenshots.
// Local-only: no network calls, no Supabase writes.

import fs from 'node:fs';
import path from 'node:path';

const SNAPSHOT_TIME = '2026-08-10T00:00:00Z';
const SEASON = 2026;
const BOOK = 'betonline';
const DEFAULT_OUT = 'data/futures-imports/betonline-2026-08-10.json';
const DEFAULT_REVIEW_OUT = 'docs/FUTURES_ODDS_BETONLINE_2026-08-10_MANUAL_REVIEW.md';
...
const WINS = [
  ...
  ['Baltimore Ravens', 11.5, 115, -145],
  ['Buffalo Bills', 10.5, -145, 115],
  ['Carolina Panthers', 7.5, 115, -145],
  ['Chicago Bears', 9.5, 100, -130],
  ['Cincinnati Bengals', 10.5, 110, -140],
  ...
  ['Seattle Seahawks', 10.5, -125, -105],
  ['Tennessee Titans', 6.5, -105, -125],
  ['Washington Commanders', 8.5, 150, -180],
];
...
function ensureExactTeams(label, rows, expectedTeams = TEAMS) {
  const seen = new Set(rows.map((row) => row.team));
  const missing = expectedTeams.filter((team) => !seen.has(team));
  const extra = [...seen].filter((team) => !expectedTeams.includes(team));
  if (missing.length || extra.length || seen.size !== expectedTeams.length) {
...
function validateRows(rows) {
  ...
  if (rows.length !== 127) throw new Error(`Expected 127 rows, got ${rows.length}`);
  ...
  ensureExactTeams('wins', rows.filter((row) => row.market_type === 'wins'), TEAMS.filter((team) => team !== 'Tampa Bay Buccaneers'));
}
...
function buildReviewDoc(rows) {
  ...
  const lines = [
    '# BetOnline Futures Manual Review - 2026-08-10',
    '',
    'Purpose: preserve the local manual transcription of the August 10 BetOnline screenshot bundle used to generate `data/futures-imports/betonline-2026-08-10.json`.',
    '',
    ...
    '## Source Screenshots',
    '',
    '- `docs/Futures_Odds/BEO_SB_0810.PNG`',
    '- `docs/Futures_Odds/BEO_Conf_0810.PNG`',
    '- `docs/Futures_Odds/BEO_Div_0810-AFC.PNG`',
    '- `docs/Futures_Odds/BEO_Div_0810-NFC.PNG`',
    '- `docs/Futures_Odds/BEO_RegWins1_0810.PNG`',
    '- `docs/Futures_Odds/BEO_RegWins2_0810.PNG`',
    '- `docs/Futures_Odds/BEO_RegWins3_0810.PNG`',
    '',
    '## Generated Import',
    '',
    `- Output: \`${DEFAULT_OUT}\``,
    ...
    '- Missing from captured screenshots: Tampa Bay Buccaneers regular-season win total and all BetOnline make-playoffs markets.',
    '',
    '## Super Bowl Winner',
    '',
  ...
  lines.push(
    ...
    '## Synthesis Caveat',
    '',
    'This artifact upgrades BetOnline from screenshot-current/exact-price-excluded to manually normalized for the markets listed above. Any market not listed here remains unavailable from BetOnline in the August 10 bundle.',
    '',
  );
...
console.log(`BetOnline 2026-08-10 manual import: ${rows.length} rows`);
