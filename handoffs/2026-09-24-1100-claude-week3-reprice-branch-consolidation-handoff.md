...
## 0. Scheduled tasks — DONE Thu 10:57
All 13 NFL tasks confirmed Ready (Twitter_Harvester Running) after being paused during the checkout switch. Toolbox → Thursday cadence ran 10:59 (availability refreshed, props intel rebuilt, live market lines synced).
...
- **SuperContest Week 3 lines received Thu 11:01 (Andy's lines supersede Westgate)** → `data/supercontest/week-03-lines.json`. Differences vs BKR 9/23: GB −5.5 (BKR −5), BUF −7.5 (−7; LAC +7.5 hook), SF −8.5 (−9), BAL −3.5 (−3; hurts BAL thesis). SuperContest 5 not built yet.
- Not done: futures review §8, Circa re-price + cheat-sheet update (no photos received), recommendation-ledger update (no tickets logged yet).
...
- **`scripts/build-player-props-intel.js` WIP rewrite** (+384/−1473, derives the current week instead of a hard-coded Week-1 map). **Works on live data:** Thu 9/24 10:59 Toolbox → Thursday run produced 13 Week 3 props (11 tier 1, 1 parlay, 2 games). It still fails 4/5 `playerPropsIntel` tests because those fixtures are Week-1 mock articles / expect the old 'Week 1' title → update the tests, then commit the rewrite. (Correction: the earlier 'live output degraded' read came from Tuesday's run + the test fixtures.) TASK_BOARD `B-props-intel-rewrite-wip`.
...
## 3b. New Thu 9/24 — TheOddsAPI quota nearly exhausted
...
verify HEAD vs origin/main (handoff commits a9a0521 + follow-up). Supabase: aambmuzfcojxqvbzhngp.

Read, in order:
1. HANDOFF.md (root index)
2. handoffs/2026-09-24-1100-claude-week3-reprice-branch-consolidation-handoff.md (last session)
...
Today:
- Scheduled tasks were re-enabled Thu 10:57 and the Thursday cadence ran 10:59 — just confirm data is fresh.
...
- THU mode: TNF ATL@GB (5:15 PT) — slot 9 island ladder (current read ATL ML +227 + Under 43; Pitts o35.5 rec yds candidate; props intel now has 13 Week 3 props). I'll paste fresh BKR/BEO TNF lines and props.
- SuperContest 5 from data/supercontest/week-03-lines.json (Andy's lines; GB -5.5 locks Thu before TNF if ATL/GB is used).
- Then: Nacua/Darnold Friday status for slots 3/4, futures review §8, Circa re-price if photos arrive.
- Early: TheOddsAPI quota (15 left) — decide plan vs. cron cadence before odds ingestion fails.
