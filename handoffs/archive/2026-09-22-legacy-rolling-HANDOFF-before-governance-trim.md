...
## Current Pick Up Here: 2026-09-21/22 (MNF) — S245: Stale-Note Corrections, Live-Tracker Week-1 Stat Leak FIXED, BEO/BKR Re-Pull Review (CRITICAL BUG FOUND, NOT FIXED), 6 MNF Tickets Logged

### Status: Tracker bug FIXED and verified. **BEO stop-label bug FIXED and verified 2026-09-22. Both BKR regressions FIXED via a new normalizer 2026-09-22. 🚑 Out/🔥 Burnt fully decoupled (ticket-level + leg-level) per Andy's live feedback, verified and regenerated 2026-09-22.** 6 MNF tickets logged ($50 at risk); MNF game still in progress (LAR 14-0, ~5:27 left in 2nd Q as of last check) so grading is pending. Still open: WAS players stuck unsettled on the Player Cheat Sheet, and locating the Graveyard list for tomorrow's post-mortem. Nothing committed.

**2026-09-22 update — BEO stop-label bug fixed (item 3 below).** `scripts/props/betonline-live-parser.mjs`'s main loop no longer `break`s when it hits a `DEFAULT_STOP_LABELS` title (e.g. "Team to Commit 1st Accepted Penalty") — it now skips that one line and keeps scanning, since real markets can (and did) follow a stop label further down the page. Verified against the real 2026-09-21 capture: re-parsing `scratch/betonline-live-2026-09-21-repull-nyg-lar.raw.json` went from 180 rows to 222 rows (+42, matching the originally-reported drop exactly) — all 10 Terrance Ferguson rows (5 markets x 2 sides) and all 4 Tyler Higbee rows (2 markets x 2 sides) recovered. `scratch/betonline-live-2026-09-21-repull-nyg-lar.parsed.json` regenerated with the fix (old buggy version backed up alongside it as `.backup-before-stoplabel-fix`) — this is now the correct go-forward baseline. Added a regression test (`tests/unit/betonlineLiveParser.test.js`, "keeps parsing player props that appear after a stop-label section") covering exactly this path; 6/6 tests passing, eslint clean. Not committed yet.

**2026-09-22 update — New live-tracker filter: "Hide Injured Legs" (Andy request, mid-session).** `scripts/generate-live-tracker.mjs` gets a dedicated leg-level filter for the 🚑 Out flag, independent of the existing "Minimize Hit & Burnt Legs" toggle (an Out leg is always also marked Burnt internally, so the two need to be OR'd together, not conflated -- either toggle can now hide an out leg on its own). New checkbox `chk-hide-injured-legs` / `toggleHideInjuredLegs()` / localStorage key `sunday_hide_injured_legs_week_<N>`, wired into the existing `applyHideFulfilledLegs()` per-leg hide loop. Also fixed a related gap while in there: the 🚑 button itself was rendering on team-side/total legs (spread/moneyline/total/team_total), where "player left the game" has no meaning -- now gated on the same `isPropLeg` check the stat-progress gauge already uses, so it only appears on player-prop legs. Verified against a full regeneration (`--week 2`): `node --check` clean on both the generator and the extracted embedded `<script>` content (the lesson from S244 -- generator syntax is not emitted-JS syntax), checkbox and function confirmed present in output, Dart's player-prop legs confirmed to keep the button, spread/total legs confirmed to have lost it. `public/live-tracker-sunday.html` and `docs/tracked-wagers/live-tracker-sunday.html` regenerated and are identical copies. Old version backed up to `scratch/live-tracker-sunday.html.backup-before-injured-filter-<timestamp>`. Not committed.

**2026-09-22 update — Both BKR extractor regressions fixed (item 3 below), via a new normalizer since no committed BKR parser exists.** Bookmaker's live capture (`bookmaker_live_dom` / `bookmaker_live_markets_v1`) is produced by an agent reading the DOM directly -- there's no checked-in extractor script to patch (unlike BetOnline's), so the fix is a new post-processing module, `scripts/props/bookmaker-live-normalize.mjs` (+ `tests/unit/bookmakerLiveNormalize.test.js`, 5/5 passing, eslint clean):
- **TD-scorer player field**: for `first_td`/`atd_1_plus`/`td_2_plus`/`td_3_plus` rows where `player` had been set to the (game-prefix-stripped) section title instead of a real name, replaced it with the real name already sitting uncorrupted in `selection`. 103/103 affected rows fixed against the real 2026-09-21 repull capture (`scratch/bookmaker-live-2026-09-21-repull-nyg-lar.json`), 0 false-positives (rows already correct were left untouched).
- **Carries misclassification**: rows whose section title matched `<Player Name> Carries` after stripping the new `"<Away> vs <Home>: "` game prefix (e.g. `"Giants vs Rams: Jaxson Dart Carries"`) but had been left in the `unknown` bucket because the prefix broke the classifier's match -- reclassified to `market: 'carries'` with the player name extracted. All 44 affected rows fixed (Dart/Skattebo/Corum/Williams, 11 each), 0 false-positives.
- Corrected artifact saved as `scratch/bookmaker-live-2026-09-21-repull-nyg-lar.normalized.json` (raw capture kept as-is alongside it); `scratch/compare-prop-market-repull.mjs` repointed at the normalized file and its output report (`scratch/prop-market-movement-2026-09-21-repull-nyg-lar.json`) regenerated -- now shows real player names and correct `carries` classification throughout. The remaining added/removed-row counts in that report (211/183 against 347 common keys) look like genuine coverage difference between the two captures (838 rows/198 sections now vs. 530 rows/57 sections in the earlier baseline), not further classification bugs -- not deep-audited beyond that.
- This normalizer is a stopgap, not the reusable extractor -- it repairs a capture after the fact, it doesn't replace the ad hoc DOM read itself. **Andy confirmed sending the Codex follow-up 2026-09-22** -- written to `handoffs/2026-09-21-1810-claude-codex-request-bookmaker-live-parser-cli.md`, asking for a real committed `bookmaker-live-parser.mjs` (CLI + exported parse functions + real test suite, mirroring BetOnline's shape) rather than continuing to patch captures after the fact.

**2026-09-22 update — 🚑 Out fully decoupled from 🔥 Burnt, per Andy's live feedback while watching MNF.** Andy reported the Jaxson Dart cards had vanished entirely after he flagged Dart's legs 🚑 Out -- root cause: `toggleLegOut()` was also setting `burntLegsState[legKey] = true`, and any burnt leg in a standard parlay/SGP auto-busts the WHOLE ticket (`isBurnt = true`), which `partitionTickets()` then re-parents into the "🔥 BURNT / ELIMINATED SLIPS" section below the live grid -- not deleted, just relocated, but Andy couldn't find it. Two rounds of fix, both shipped and verified:
- **Round 1 (ticket-level):** added `nonInjuryBurntLegsInTicket` (burnt legs minus any that are also Out) and changed the standard-parlay busting check to key off that instead of the raw burnt-leg list, so an Out-only leg no longer busts the whole ticket.
- **Round 2 (leg-level, supersedes Round 1's framing):** Andy then asked for more -- not just "don't bust the ticket," but "don't freeze the leg either": *"I see the Jaxson Dart tickets, but would like to untoggle the Burnt setting for those legs, until the end of the game when the leg becomes officially Burnt. That way I can see how close those parlays get during the game."* So `toggleLegOut()` was rewritten to be **purely informational** -- it no longer touches `burntLegsState` at all, only `outLegsState`. This meant three independent rendering code paths that had all silently assumed "Out implies Burnt" needed fixing so an Out-only leg doesn't fall through to a blank/reset default state and lose its indicator:
  - The periodic live-stat-refresh poller (~line 7069): now unconditionally syncs the `leg-out` CSS class every tick regardless of which pace branch fires, so an injured-but-not-burnt leg keeps its purple tint while still showing real pace/progress.
  - The Player Cheat Sheet's `sub-gauge-item` renderer (~line 6107): `sg-out` was gated on `isLegBurnt && isLegOut` (dead now that the two are decoupled) -- changed to `isLegOut && !isLegBurnt`; added a dedicated `else if (isLegOut)` branch in the `pstatEl` chain (after Burnt, after Hit, before Final) showing "🚑 Out" as a tag alongside the real live `currentVal / target`, not a frozen 0.
  - The main `render()` per-leg loop (~line 8958): added a new `else if (isLegOut)` branch between the existing `isLegHit` and default-reset branches -- sets the `leg-out` class/🚑 icon/button active-state without touching `cardStat`/`cardBar`/`paceBadge`, leaving those to the periodic refresh and cheat-sheet renderer to populate with real numbers. Also had every other branch (`isLegHit`, the final default) explicitly clean up the `leg-out` class and the Out button's active state, since these are now two fully independent toggles rather than one implying the other.
- **Verification:** `node --check` clean on the generator and (per the standing S244 lesson) on the extracted embedded `<script>` block of a fresh regeneration; grepped the emitted script to confirm the new `isLegOut` branches, `btnOutLegOnly`/`btnOutLegHit`/`btnOutLegReset` button-state syncs, and the corrected `sg-out` toggle all made it through the template-literal escaping intact. **eslint on `scripts/generate-live-tracker.mjs` timed out 4 times in a row this session (up to the 180s device_bash cap) and could not be run to completion -- unresolved reliability issue, `node --check` was the only completed gate.** `public/live-tracker-sunday.html` and `docs/tracked-wagers/live-tracker-sunday.html` regenerated together from the same run and confirmed byte-identical (md5 `563844ae5cffe4337545faecdf505e4b`). Old versions backed up to `scratch/live-tracker-sunday.html.backup-before-out-decouple-render-v2` and `scratch/live-tracker-sunday-docs.html.backup-before-out-decouple-render-v2` (a `docs/tracked-wagers/live-tracker-sunday.html` truncated by an earlier timed-out regeneration mid-session is now fixed by this regeneration). **Andy's own words to preserve the intent going forward: 🚑 Out is a live, non-final flag -- the leg keeps tracking real progress until Andy (or normal grading) marks it 🔥 Burnt himself once the game actually ends.**

**2026-09-22 update — 9 stuck-PENDING WAS @ DAL player-prop legs graded; found a MUCH larger ungraded backlog while doing it.** Andy noticed a couple of WAS players still showing unsettled on the Player Cheat Sheet from Sunday's (2026-09-20) WAS @ DAL game (final: DAL 37, WSH 20) and asked for a review/close-out. Pulled the real final box score from ESPN's summary API and graded all 9 stuck player-prop legs directly in `data/official-picks/user-placed-wagers-2026.json` (`status` + `actual_stat` fields): Antonio Williams 26+ rec yds LOST (24), Antonio Williams 2+ rec WON (3), CeeDee Lamb 1+ TD WON (2), Dak Prescott 2+ pass TD WON (4), Stefon Diggs 5+ rec WON (5 -- **note: Diggs is on Washington now**, not a stale-roster false alarm), George Pickens 5+ rec WON (6), Javonte Williams 3+ rec WON (3), Jacory Croskey-Merritt 52+ rush yds LOST (43), Jacory Croskey-Merritt 1+ TD LOST (0). **Did not touch ticket-level `status`/`result` fields** -- every one of these legs sits on a multi-game parlay/round-robin with other legs still pending from other Sunday games, so the overall ticket win/loss isn't resolvable yet.

**Bigger finding, flagged for Andy, not yet actioned:** while scoping this, a full scan of the wagers file found **172 total PENDING legs**, of which **131 are from Sunday 2026-09-20 games that are almost certainly all final by now** (IND@KC 30, MIA@SF 17, PHI@TEN 10, LV@LAC 10, GB@NYJ 9, SEA@ARI 9, JAX@DEN 9, WAS@DAL 8 remaining team-line legs, MIN@CHI 7, CLE@TB 5, CIN@HOU 5, CAR@ATL 4, PIT@NE 4, NO@BAL 3) -- the remaining 41 are tonight's still-live NYG@LAR MNF game. **This looks like the entire Week 2 Sunday slate never got graded**, not just the couple of WAS players Andy happened to notice. Grading it properly means real ticket-level settlement math across multi-leg parlays spanning many games -- a substantial task that deserves Andy's explicit go-ahead (and probably its own session) rather than a silent bulk pass. **Next session: surface this to Andy directly and ask whether/how he wants it done** (leg-status-only vs. full ticket settlement).

**2026-09-22 update — "Graveyard" list of proposed-but-unused prop legs: confirmed it does NOT exist anywhere in this repo.** Andy asked to review it ahead of tomorrow's Week 2 post-mortem. Searched case-insensitively for "graveyard" and several synonyms (bench list, considered-but-not-placed, excluded/candidate/rejected leg, leg pool) across the entire repo (excluding `node_modules`/`.git`) -- zero hits anywhere except this session's own new HANDOFF entries referencing Andy's request. **No such file or mechanism currently exists under any name I could find.** Flagging honestly rather than guessing -- next session should ask Andy directly what he's recalling (a different session/tool he used informally, or something he wants built going forward from here) rather than assume a file was simply missed.

**2026-09-22 update — full project handoff written for a fresh/second Claude team, at Andy's explicit request.** He asked for "a full and complete handoff... on everything we have built over the last few days" so a new team can orient and resume. Wrote `handoffs/2026-09-22-0240-claude-to-claude-full-project-handoff.md` -- a standalone orientation document (architecture cheat sheet, standing constraints, the live prop-capture/live-tracker subsystem deep dive, betting-strategy knowledge pointers, current session state, and a recommended next-steps order) meant to be read once by someone with zero prior context, complementing rather than replacing this rolling HANDOFF.md log. Also delivered a copy to Andy directly in the conversation.

1. **TWO STALE HANDOFF NOTES CORRECTED IN PLACE (see items 1 and 5 of the older entries below).** Both actively misled this session before being caught:
   - `device_bash` has **outbound network** (verified: ESPN scoreboard returns HTTP 200 from the bridge). The 2026-09-18 note claiming otherwise is false.
   - `live_tracker_player_out_flag` is **already built and live** (🚑 button per leg, `toggleLegOut()`, purple `sg-out` styling, localStorage-persisted). The 2026-09-20/21 note calling it an open gap is false. **ESPN's live `injuries` block is pregame inactives only and never updates in-game — the manual flag is the only mechanism; do not hunt for an auto-detect feed.**
   - **Lesson: correct a stale HANDOFF note the moment it's disproven.** Both cost real time tonight.

2. **FIXED — live tracker was showing WEEK 1 stats for Rams players mid-game.** `scripts/generate-live-tracker.mjs` unconditionally injected two hardcoded Week 1 event ids: `401872656` (SEA/NE) and `401872657` (**LAR 7 @ SF 27**). The stat merge OVERWRITES rather than accumulates, and a live boxscore only contains athletes who have already recorded a stat — so any Rams player without a touch yet kept his Week 1 line in `athleteStatsMap`. Live effect: Kyren Williams showed 11 car / 41 rush yds and Stafford showed 15/25 for 155 while the game was 0-0. NYG legs were unaffected (no Week 1 NYG event in the fetch set). Removed both ids with an explanatory comment; backup at `scratch/_generate-live-tracker.mjs.backup-before-eventid-fix`. Regenerated and verified the Week 1 file was not re-fetched. **Rule: never seed `eventIds` from a different week than the one being rendered.**

3. **BEO/BKR prop re-pull review — REVIEW ONLY, nothing fixed. One CRITICAL bug:**
   - **CRITICAL — `scripts/props/betonline-live-parser.mjs` terminates early and silently drops 42 rows.** `DEFAULT_STOP_LABELS` breaks the main loop at `Team to Commit 1st Accepted Penalty` (raw line 1112), but 208 lines of legitimate markets follow it. Lost: **all 5 Terrance Ferguson markets and both Tyler Higbee markets** (14 player-prop rows) plus 14 game-level markets (Total FGs/Sacks/TDs, the "Will there be..." set). The new BEO artifact is the designated go-forward baseline and is **missing players Andy has live action on**. Fix: treat stop labels as skip-and-continue, not `break`; add a regression test for props appearing after a stop label. **None of the 5 existing tests cover this path, which is why it shipped green.**
   - Countdown patch is **narrowly correct** (matched all 116 countdown lines, zero misses) but brittle to format variants (`Tomorrow in ...`, 2-segment `MM:SS`) which would fail silently as misalignment.
   - ATD split **holds cleanly**: `atd_1_plus` (list section) vs `atd` (Yes/No), no collision. Row counts reconcile (180 = 180). The 10 `unknown` rows are all game-level specials, no ordinary player props.
   - BEO capture is incomplete: `atd_1_plus`/`first_td` captured only **6 players each** because the page collapsed the lists behind `SHOW MORE` and extraction never expanded them (parser correct, capture short).
   - **BKR regression — TD-scorer `player` field destroyed.** All `first_td`/`atd_1_plus`/`td_2_plus`/`td_3_plus` rows have `player` = the section title (`'Player To Score 1st Touchdown'`). Real name survives in `selection`, so recoverable.
   - **BKR regression — `carries` misclassified, not removed.** All 44 carries rows are present but typed `unknown` because section titles now carry a `Giants vs Rams: ` prefix that the classifier doesn't strip (the comparison script strips it; classification upstream does not).
   - **The movement report is 94% / 92% false positives.** Of 329 removed / 313 added: TD player-field regression 109/103, carries misclassification 44/0, ladder re-anchor (BKR shifted its alt-line grid by 1 yard — real page change, not odds movement) 155/169, `Total Receptions` grid mislabelled as `rec` 0/16. **Only the 201 common keys and their 151 odds moves are trustworthy.**
   - **Genuine movement confirmed** (same-line): Singletary receptions ladder shortened hard (6+: +2010 → +917), Kyren all 7 rungs, Mooney 2+ (+107 → −122), Mumpfield 2+ (+102 → −120), Fields rec yds 27+ (+108 → −106). Counter-direction: Ferguson rec 3+ **lengthened** (−101 → +109).
   - Tests genuinely pass: `node node_modules/vitest/dist/cli.js run tests/unit/betonlineLiveParser.test.js` → 1 file, 5 tests.

4. **Roster verification is now a standing rule (Andy-corrected, twice).** Claude twice called real current players "data contamination" from stale training knowledge: Isaiah Likely / Patrick Ricard / Najee Harris are **Giants** (John Harbaugh is NYG HC in 2026), Myles Garrett is a **Ram**, and Aaron Donald **unretired** (1yr/$20M, returned Week 2 on ~30 snaps). Full rule written to `docs/BETTING_LESSONS_LEARNED.md` and project memory: cross-check `data/nfl-rosters/roster-map-latest.json` first, verify anything surprising with live search, never conclude "contamination" without it. **Also: that roster map holds only ~815 players (~25/team) and does NOT cover most defensive backs** — Greg Newsome II is absent from it; his team came from the giants.com injury report.

5. **Prop projection model FAILED validation — do not use its edge numbers.** Backtested against 73 gradeable legs: **Brier 0.2583 vs 0.2500 for the base rate (skill −3.3%)**, calibration near-inverted (65-80% bucket hit 40%; 35-50% bucket hit 59%). Scripts kept at `scratch/prop-screen-2026-09-21-nyg-lar.py` and `scratch/backtest-prop-model.py`. What survived is empirical, not model-based: yardage props are bimodal (winners clear by +45.7/+32.8 avg, losers miss by −27/−17), receptions are tight (±1-3), and the **−120..−100 band is where the bleeding is (37 legs, 45.9%, below breakeven)** while ≤−200 went 12/15. Full writeup in `docs/BETTING_LESSONS_LEARNED.md`.

6. **6 MNF tickets logged to `data/official-picks/user-placed-wagers-2026.json` (56 → 62 tickets), $50 at risk / $2,844 max.** BKR 739096481 side+total ($25, +237); BEO 396980322 (+14700), 396981556 (+5400), 396982644 (+11700), plus two BEO 7-leggers (+21700, +1200) still **missing ticket numbers**, and 396981556/396982644 missing per-leg prices. All rendered and verified in the tracker. **Jaxson Dart is on 4 of 6 tickets ($20) and left the game injured in Q1** — mark those legs 🚑 Out if he doesn't return.

7. **Data-integrity notes for the pipeline:** the sportsbook board and roster map spell players differently (`Cameron Skattebo` vs `Cam Skattebo`, `Odell Beckham` vs `Odell Beckham Jr.`, `Thomas Fidone` vs `Thomas Fidone II`) — exact-match roster gates silently fail; `scratch/verify-mnf-cards.py` resolves via first-initial+surname. **nflverse codes the Rams as `LA`, the roster map as `LAR`** — filtering stats on 'LAR' returns zero rows and looks like missing data. BetOnline ticket-id format is NOT a reliable book fingerprint (3969xxxxx here vs 997920156 previously) — ask, don't infer.

### Uncommitted this session (on top of the existing large dirty tree)
- `scripts/generate-live-tracker.mjs` — Week-1 event-id leak fix (item 2)
- `data/official-picks/user-placed-wagers-2026.json` — 6 MNF tickets
- `docs/BETTING_LESSONS_LEARNED.md` — roster-verification rule + model-validation-failure writeup
- `HANDOFF.md` — this block + two stale-note corrections + 2026-09-22 fix updates
- `public/live-tracker-sunday.html`, `docs/tracked-wagers/live-tracker-sunday.html` — regenerated
- `scratch/` — prop screen, backtest, card verifier, tracker/HANDOFF/wagers backups
- `scripts/props/betonline-live-parser.mjs`, `tests/unit/betonlineLiveParser.test.js` — 2026-09-22 stop-label fix + regression test
- `scripts/props/bookmaker-live-normalize.mjs` (new), `tests/unit/bookmakerLiveNormalize.test.js` (new) — 2026-09-22 BKR regression fixes
- `scratch/betonline-live-2026-09-21-repull-nyg-lar.parsed.json` — regenerated with the BEO fix (old version backed up as `.backup-before-stoplabel-fix`)
- `scratch/bookmaker-live-2026-09-21-repull-nyg-lar.normalized.json`, `scratch/bookmaker-sgp-live-2026-09-21-nyg-lar.normalized.json` (new) — normalizer output
- `scratch/compare-prop-market-repull.mjs` — repointed at the normalized BKR file
- `scratch/prop-market-movement-2026-09-21-repull-nyg-lar.json` — regenerated with corrected data
- `scripts/generate-live-tracker.mjs` — new "Hide Injured Legs" filter + 🚑 button gated off team-side/total legs
- `public/live-tracker-sunday.html`, `docs/tracked-wagers/live-tracker-sunday.html` — regenerated again with the injured-filter feature (old version backed up to `scratch/`)
- `scripts/generate-live-tracker.mjs` — 🚑 Out fully decoupled from 🔥 Burnt at both the ticket level (`nonInjuryBurntLegsInTicket`) and the leg level (`toggleLegOut()` rewrite, periodic-refresh `leg-out` sync, cheat-sheet `sg-out`/`pstatEl` fix, new `render()` `isLegOut` branch)
- `public/live-tracker-sunday.html`, `docs/tracked-wagers/live-tracker-sunday.html` — regenerated again with the Out/Burnt decoupling (byte-identical, md5 `563844ae5cffe4337545faecdf505e4b`); this regeneration also fixes a `docs/tracked-wagers/live-tracker-sunday.html` copy that had been left truncated by an earlier timed-out regeneration call mid-session
- `data/official-picks/user-placed-wagers-2026.json` — 9 WAS @ DAL player-prop legs graded with real final stats (leg-level `status`/`actual_stat` only, ticket-level settlement left untouched)
- `handoffs/2026-09-22-0240-claude-to-claude-full-project-handoff.md` (new) — full orientation handoff for a fresh Claude team, per Andy's explicit request

### Next session should start with
1. **Grade the MNF slate once the game goes final** (was LAR 14-0, ~5:27 left in the 2nd Q, as of the last check this session; a `send_later` reminder is scheduled to fire at 2026-09-22T03:24:00Z / ~8:24pm PT, trigger `trig_01U22thTSJzT6TCaJbyvrKUs`). Jaxson Dart exited with a knee injury in Q1 and is confirmed NOT returning (Jameis Winston in at QB). Andy has already flagged Dart's legs 🚑 Out himself in the live browser session; per his explicit request that flag is now purely informational (does not bust the ticket or freeze the leg) -- final grading of those legs still needs to happen once the game (and Dart's replacement-drive stat lines) are actually final.
2. **Review and close out stuck WAS players on the Player Cheat Sheet** (Andy: "I still see a couple of players from WAS that did not settle after their game ended yesterday") -- not yet started; need to identify which Sunday 2026-09-20 WAS game/players and why their legs never resolved out of pending.
3. **Locate and review the "Graveyard" list of proposed-but-unused prop legs** ahead of tomorrow's Week 2 post-mortem (Andy: "Do we still have a Graveyard list...I would like to review those when we do the week 2 post-mortem tomorrow") -- not yet started; need to confirm whether such a file/list exists in the repo and where.
4. The BEO stop-label bug and both BKR regressions are fixed (see above), and the Codex follow-up for a reusable `bookmaker-live-parser.mjs` CLI has been sent (`handoffs/2026-09-21-1810-claude-codex-request-bookmaker-live-parser-cli.md`) -- check whether Codex has responded.
5. **eslint on `scripts/generate-live-tracker.mjs` has become unreliable** (4 consecutive timeouts at the 180s cap late this session, after passing cleanly earlier in the same session) -- worth a clean retry next session; `node --check` on the generator + the extracted embedded `<script>` remains the fallback verification gate.
6. **Ask Andy how he wants the 131-leg Week 2 Sunday grading backlog handled** (see the dated entry above) -- 9 WAS @ DAL player-prop legs are already graded, but that game's team-line legs and 12 other Sunday games' worth of legs (IND@KC, MIA@SF, PHI@TEN, LV@LAC, GB@NYJ, SEA@ARI, JAX@DEN, MIN@CHI, CLE@TB, CIN@HOU, CAR@ATL, PIT@NE, NO@BAL) are still sitting PENDING despite those games being final. Don't bulk-grade silently -- this touches real multi-leg parlay settlement math.
7. **The "Graveyard" list Andy asked about does not exist anywhere in this repo** (confirmed via exhaustive search, see above) -- ask him directly what he's recalling before assuming it needs to be built from scratch.
8. A comprehensive standalone orientation doc now exists at `handoffs/2026-09-22-0240-claude-to-claude-full-project-handoff.md` for anyone (a fresh Claude team, or just a session that wants a faster architecture/constraints refresher than reading this whole file) picking this repo up without prior context.

Standing constraints unchanged (git guardrail repealed; Supabase writes and paid synthesis need per-change authorization).

---

## Current Pick Up Here: 2026-09-21 (evening) — Live Prop Capture Verified + Toolbox Capture Buttons Shipped; Handing Off to Build MNF Stacks (IN PROGRESS)

### Status: Codex-team live extraction round-tripped and verified end-to-end; two new Toolbox buttons shipped; next session's job is to actually build tonight's (NYG @ LAR, MNF, Week 2) Island-game stacks from the data already captured. Nothing committed — see "Uncommitted this session" below.

1. **BetOnline live parser (`scripts/props/betonline-live-parser.mjs`) — verified clean after 2 rounds of Codex fixes.** Round 1 caught two real bugs (row-count didn't reconcile 166 vs claimed 170; `Anytime Touchdown Scorer` rows silently dropped). Round 2's fix introduced a NEW bug I caught before accepting it: the fix reused market key `atd` for both the outright `Anytime Touchdown Scorer` list AND the `"Score a Touchdown?"` Yes/No market — two structurally different bet types colliding under one key, with inconsistent player/team parsing between them. Sent Codex a formal review-prompt handoff (`handoffs/2026-09-21-2011-claude-codex-review-prompt-betonline-atd-collision.md`) asking for a market-key split + consistent team parsing. **Round 3 (final, accepted):** Codex split it into `atd_1_plus` (outright list, matches Bookmaker's own naming convention) vs `atd` (Yes/No), and normalized player/team parsing across both code paths. I re-ran the exact collision repro myself (not just their claimed numbers) and confirmed: 4/4 tests pass, `atd_1_plus`/`atd` now come back as genuinely distinct rows with `team` populated on all of them. **This one's good — no more open issues on it.**

2. **Bookmaker SGP live extraction (`scratch/bookmaker-sgp-live-2026-09-21-nyg-lar.json`) — verified clean, accepted as-is.** 530 rows / 57 sections for tonight's NYG @ LAR matchup. I independently re-ran their verification command, confirmed market-count sums to exactly 530 with zero duplicate rows and zero malformed odds, and caught one real issue (the original `*_sgp` labels on ordinary spread/total/moneyline sections were misleading — `game_sgp`/`first_half_sgp`/etc. were NOT same-game-parlay legs, just standard game/period lines mislabeled). Codex's follow-up renamed those to `game_lines`/`first_half_lines`/`first_quarter_lines`/etc. and relabeled the schema `bookmaker_live_markets_v1` — re-verified clean, no leftover `*_sgp` keys, row math still exact. **This is the live market-line data available for tonight's stacks** — `atd_1_plus`, `td_2_plus`, `td_3_plus`, `first_td`, `carries`, `rush_yds`, `rec`, `rec_yds`, `pass_*`, `kicking_points`, plus the ordinary game/period lines.

3. **IMPORTANT for next session: BetOnline has NO saved raw market-line data file for tonight's game.** Both BetOnline handoffs explicitly noted no JSON artifact was saved from the live browser run — only the parser + its unit-test fixtures exist in the repo. Don't go looking for a `betonline-live-*.json` file for NYG@LAR; it doesn't exist yet. The only real captured lines on disk right now are the Bookmaker artifact from item 2. Building tonight's stacks with only one book's lines is fine — the Island-game template doesn't require multi-book comparison, just correlated legs from A board — but flag it to Andy if he expects both books' lines to be there.

4. **Toolbox: two new "agent-assisted" live-capture buttons shipped, currently uncommitted.** Added a "Live Sportsbook Capture (agent-assisted)" section (`scripts/toolbox-dashboard.html`) under the **Diagnostics tab** (not Weekly Cadence — Andy initially couldn't find it because the Toolbox's top nav shows only one `.section-card` category at a time via `setCategoryFilter()`; confirmed by driving the live page directly with the browser pane, not by guessing). Two buttons: "Open BetOnline NFL Props" and "Open Bookmaker.eu NFL Lines" — both just open the book's live page in Andy's own logged-in browser (`runTask` cases `launch-betonline-props` / `launch-bookmaker-props` in `scripts/toolbox-app-server.mjs`), then log a reminder to ask an agent to capture+parse. **Explicitly NOT a headless/unattended scraper** — Andy chose the human/agent-assisted path over a Playwright-with-saved-cookies approach specifically because these are real-money sportsbook accounts and unattended login/scraping risks account flags (same concern that killed the Odds Terminal evaluation earlier). Also updated `scripts/props/README.md` with a "Live capture (agent-assisted)" section documenting the workflow and the canonical output path (`data/generated/props/<book>-live-<date>-<away>-at-<home>.json`, not `scratch/`).

5. **Known gap, not yet fixed:** Andy actually wants ALL matchups' prop lines in one pass (originally pointed at `/sportsbook/props`, which Codex's own handoff already found doesn't expose player-prop markets — the working route is the `/sportsbook/futures-and-props/nfl-game-props/` hub, which lists every matchup with a working per-game props URL). **I cannot drive this myself** — my browser pane refuses betonline.ag outright ("not allowed due to safety restrictions", a categorical block on gambling-site domains). The parser already supports a multi-game payload (`parseBetOnlineBoardPayload({events:[...]})`) so the mechanics for "all matchups, one file" already exist; only the actual page-by-page capture needs a human or Codex driving it. Andy decided to defer this (Codex is offline for a few hours) and handle it later — **do not attempt to browse betonline.ag/bookmaker.eu yourself in the next session either**, same restriction will apply.

6. **Still-open, not yet sent:** I offered to send Codex a follow-up asking them to commit a reusable `bookmaker-live-parser.mjs` CLI (mirroring BetOnline's `--in`/`--out` convention) since Bookmaker's extraction is still "a live extraction pass, not a committed reusable parser" per their own handoff notes. Andy didn't confirm sending it before we moved on — ask him if he still wants that sent.

### Uncommitted this session (git working tree, on top of already-large pre-existing dirty state)
- `scripts/props/betonline-live-parser.mjs`, `tests/unit/betonlineLiveParser.test.js` — final accepted version (round 3, `atd_1_plus`/`atd` split)
- `scratch/bookmaker-sgp-live-2026-09-21-nyg-lar.json` — final accepted version (`*_lines` rename)
- `scripts/toolbox-app-server.mjs`, `scripts/toolbox-dashboard.html`, `scripts/props/README.md` — new capture buttons + docs
- `handoffs/2026-09-21-2011-claude-codex-review-prompt-betonline-atd-collision.md` — the review-prompt sent to Codex (now resolved)

### Next session's actual job
Build tonight's (NYG @ LAR, Monday Night Football, Week 2) player-prop parlay stacks using the Island-game 3-tier template (`docs/BETTING_LESSONS_LEARNED.md` — no wait, template itself is in project memory `nfl_dashboard_island_game_parlay_template.md`, not a repo file — read that first) against the Bookmaker lines already captured in `scratch/bookmaker-sgp-live-2026-09-21-nyg-lar.json`. See the resume prompt in chat for the full brief.
...
5. **~~New open feature gap~~ — CLOSED, see correction.** (Original note: the tracker has no concept of a player leaving the game injured/inactive, after Alec Pierce exited mid-game and two tickets kept reading on-pace.) **CORRECTED 2026-09-21 (S245): this feature was BUILT after this entry was written and the note was never updated — it is live in the tracker today.** Every leg renders a 🚑 button (`btn-leg-out`, 338 of them in the current Week 2 tracker) wired to `toggleLegOut()`, which marks the leg Out **and** Burnt so it drops out of ticket math immediately, uses distinct purple styling (`#8B5CF6`, `sg-out` vs `sg-burnt`), persists to `localStorage` under `sunday_out_legs_week_<N>`, guards with a confirm if the leg already grades HIT or PUSH, and clears conflicting Push/checked state. ESPN's live summary `injuries` block is PREGAME INACTIVES ONLY and never updates for in-game injuries, so the manual flag is the correct and only mechanism — do not go looking for an auto-detect feed. (Note: the original entry claimed this was tracked in `.atlas-bridge/memory.json` as open_task `live_tracker_player_out_flag` — it never was. Verified in S245: no such task id exists in that file.)
...
1. **Friday cadence ran successfully from Andy's own terminal** (1124 availability events / 32 teams, 76 projected-starter signals, 22 players flagged for Human Review). ~~Note: the Cowork `device_bash` shell has NO outbound network...~~ **CORRECTED 2026-09-21 (S245): this is FALSE and cost a later session real time.** `device_bash` DOES have outbound network — verified by fetching the ESPN scoreboard from the bridge shell and getting HTTP 200. Live-data scripts (including `generate-live-tracker.mjs`) run correctly from the bridge and pull fresh data. Do not route around the bridge on this assumption.
