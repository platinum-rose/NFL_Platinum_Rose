...
| F-11 | Intel search tool (`search_intel`) | P1 | Mid-conversation intel lookup for BETTING agent. **Phase 1** — `search_intel` tool in `agentTools.js` + `searchResearchIntel(query, opts)` in `supabase.js`; `ilike` on `title \|\| summary`; params: `query`, `source?`, `hours?` (default 168), `limit?` (default 5); returns matching notes + attached pick signals. **Phase 2** — migration `010_research_intel_body.sql` adds `body text` + tsvector FTS index; ingest agent follows article URLs, strips HTML, stores up to 4000 chars; switches to Postgres full-text search. Enables: *"What did Action Network say about the Chiefs this week?"* Unblocked — RSS feeds live (DS-4 done). |
