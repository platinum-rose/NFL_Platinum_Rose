...
1. Dry run against 1–2 recent episodes per show; review actual output with Andy before trusting it at scale.
   ...
   **Bug caught live during the first pilot run (2026-07-20):** `selectBackfillTargets` originally only sorted candidates by `pub_date`, with no NFL-relevance check — it picked a "WORLD CUP FINAL BETTING PREVIEW: Spain vs. Argentina" episode on Sharp or Square for paid diarization (already-ingested backlog episodes predate or slipped past `podcast-ingest.js`'s own title filter, which only runs at discovery time). Fixed by extracting that filter into `agents/lib/nfl-relevance.js` (shared with `podcast-ingest.js`, which now imports it too) and applying it at selection time as well — 4 new tests cover the exclusion, that it doesn't count against a show's `--limit-per-show` cap, and that an explicit `--episode` override still bypasses it. Left the already-running M6 pilot job alone rather than interrupting it (its already-submitted AssemblyAI jobs bill regardless of whether the local process is killed) — this fix applies to the next run, including the eventual task 9 full backfill.
