...
Status: model-ready local evidence packet. No paid model/API call was made. No Supabase write, official-pick approval, production recommendation persistence, portfolio mutation, or open-parlay change was made.

## Current Decision Point

Choose one of these before the frontier-model run:

1. Normalize or manually review the July 29 BetOnline screenshots first, then allow exact BetOnline prices in the synthesis.
2. Run the synthesis now with BetOnline marked screenshot-current but exact-price-excluded, using BetUS and Bookmaker/BKR as current placeable-price sources.
...
| BetOnline July 29 screenshots | Current screenshots exist, last written 2026-07-29 9:34-9:41 PM local | Use as screenshot-current market evidence. Do not use exact BetOnline prices unless manually reviewed or normalized. |
| `data/futures-imports/betonline-2026-07-14.json` | Stale structured import, 128 rows | Excluded from current exact-price use unless reconciled to July 29 screenshots. |
...
BetOnline screenshot files to normalize if exact prices are needed:
...
Manual visual spot-check: `BEO_SB_0729.PNG` is readable enough for extraction, but all nine screenshots should be normalized or manually checked before exact BetOnline prices are treated as actionable.
...
| M6 podcast deep dives | `docs/podcast-transcript-deep-dives/index.json`, generated `2026-07-30T07:40:54.220Z`, 57 episodes | Current transcript narrative base. Latest episode is the July 23 BettingPros futures-card episode with 48 beats. |
| Podcast ad/legal cleanup | `scripts/build-podcast-transcript-deep-dives.js` and `agents/lib/speaker-attribution.js` checks passed; regenerated output committed | Expanded hard promo/legal scan is clean; remaining sportsbook mentions are price/context references. |
...
| Training camp snapshot | `data/training-camp/2026/latest.json`, generated `2026-07-30T03:46:07.745Z` | Intel context: all 32 teams represented, 16 items, 12 teams with intel, 20 teams without new local intel, 3 high-priority items. One RSS feed issue remains for Football Outsiders. |
...
- Treat current BetUS and Bookmaker/BKR as current placeable-price evidence.
- Treat BetOnline July 29 screenshots as current but exact-price-excluded unless normalized or manually reviewed.
...
Recommended next move: normalize or manually review the BetOnline July 29 screenshots only if exact BetOnline prices are important to the recommendation set. Otherwise, proceed to an approval-gated frontier-model synthesis using current BetUS and Bookmaker/BKR prices, with BetOnline exact prices excluded.

## Resume Prompt

Resume Platinum Rose NFL in `E:\dev\projects\NFL_Dashboard`. Read `HANDOFF.md`, `HANDOFF_PROMPT.md`, `WORKING-CONTEXT.md`, `TASK_BOARD.md`, `docs/FUTURES_SYNTHESIS_SOURCE_READINESS_2026-07-30.md`, and `docs/FUTURES_PORTFOLIO_FRONTIER_SYNTHESIS_PACKET_2026-07-30.md` first. Current objective is a maximum-effort frontier-model futures portfolio narrative and recommendation synthesis, using current accepted local sources and excluding DraftKings/FanDuel bet-slip parser work plus weekly/live props plumbing from the current focus. Verified state: latest source audit is PASSABLE with Current 2 / Review 17 / Stale 0 / Blocked 0 / Missing 0 / Context 7 / Inference 1; podcast deep dives were regenerated to 57 episodes after expanded ad/legal filtering; BetUS and Bookmaker/BKR July 29 imports are current; BetOnline has July 29 screenshots but stale structured rows must not be used as exact-price source of truth. Immediate next step: decide whether to normalize BetOnline screenshots before the model run or run with BetOnline exact prices excluded. Guardrails: no paid model/API call, no Supabase write, no official-pick approval/proposal persistence, no production recommendation persistence, and no open-parlay changes without explicit approval.
