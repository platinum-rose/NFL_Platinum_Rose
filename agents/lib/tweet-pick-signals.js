...
Return ONLY JSON: {"picks":[...]}. One entry per distinct bet the AUTHOR is recommending or has placed:
{
  ...
  "bet_type": "spread|total|moneyline|player_prop|team_total|futures|survivor|parlay_leg",
...
const BET_TYPES = new Set(['spread', 'total', 'moneyline', 'player_prop', 'team_total', 'futures', 'survivor', 'parlay_leg']);
...
export function buildTextPickSignalRows(picks, { noteId, eventRef, sourceLabel, author }) {
  return (Array.isArray(picks) ? picks : [])
    ...
    .map((p) => {
      ...
      const teamOrMarket = betType === 'player_prop' || (betType === 'parlay_leg' && market)
      ...
      return {
        ...
        event_ref: eventRef,
        confidence: 0.55, // model-extracted from prose; below the 0.65 note confidence
      };
    });
}
