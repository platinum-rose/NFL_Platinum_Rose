...
export const AD_COPY_RE = /\b(amazon|pharmacy|orderly\s*meds|orderlymeds|zepbound|tirzepatide|lilly\.com|pedigree|dog food|vitamin good bites|free delivery|healthcare|promo|bonus bet|bonus bets|bonus rewards|terms and conditions|download the app|download .* app today|subscribe|youtube|apple podcasts|spotify|hard rock bet|gambling problem|gamble responsibly|iheart podcast|iheart ?radio app|guaranteed human|paid for by|must be 21|call 1-800|call 1-833|1-800-?\s*gambler|1-833-?\s*playwise|not a cash offer|wix|business idea|sign to take action|cash back deals|credit card rewards|apollo|grainger|american express|americanexpress\.com|mx gold|points are piling up|membership rewards|spinquest|spin quest|mcdonald'?s|refreshers|popping boba|free to play social casino|wherever you get your podcasts|wherever you download|podcast network|terms and points cap apply|zbiotics|use code|sign up with code|at checkout|welcome offer|offer limited time|new draftkings customers|new customer bonus|turn five bucks|spend (?:five|5) bucks|the crown is yours|draftkings network|dkng\.?co|predictions offer void|checkout is offered by your banks|advisory services provided|securities offered by|topgolf|funpass|ally bank|member fdic|bank of america|b of a rewards|what would you like the power to do|t mobile|total wireless|official wireless partner|official sportsbook partner|presenting sponsor|today'?s show is brought to you|nhtsa|u\.s\. transportation secretary|nora jones|playing along|paul'?s best podcast|paul virzi|paul verze|will ferrell|big money players|first america|levels to this|tareka foster|sheryl swoopes|wnba analysis|alzheimer|burden of guilt)\b/i;
export const AD_CONTEXT_RE = /\b(?:let'?s do a quick (?:word|message|break)(?: here)? from(?: our friends at )?hard rock|quick (?:word|message|break) from hard rock|friends at hard rock|presented by hard rock|offered by seminole hard rock digital|hard rock (?:is rolling out|also offers|offers|has it covered|has it coming covered|is also live in)|new promos|terms for details subject to change|top .*sportsbook app)\b/i;
...
function speakerStats(utterances) {
  ...
  for (const u of utterances) {
    ...
    const isAd = AD_COPY_RE.test(text) || AD_CONTEXT_RE.test(text);
    current.turns += 1;
