...
// Replaces the GAS "alertLineMoves" trigger. Runs hourly via systemd.
...
function fmtBook(key) { return BOOK_LABELS[key] || key; }
...
function buildEmail(alerts) {
  ...
  const subject =
    `NFL Line Alert -- ${alerts.length} move${alerts.length > 1 ? 's' : ''}` +
    (highCount ? ` (${highCount} HIGH)` : '') +
    ` -- ${ts}`;

  const rows = alerts.map(a => {
    const icon = a.significance === 'HIGH' ? '[HIGH]' : '[MEDIUM]';
    return (
      `${icon} ${fmtGame(a)} | ` +
  ...
  const htmlRows = alerts.map(a => {
    ...
    return `
      <tr style="background:${bg}">
        ...
        <td style="padding:8px 10px;font-family:monospace">${fmtLine(a.from_line, a.type)} -> ${fmtLine(a.to_line, a.type)} (${fmtMove(a.movement)})</td>
        <td style="padding:8px 10px;color:#666;font-size:12px">${fmtTime(a.detected_at)}</td>
      </tr>`;
  }).join('');

  const html = `<!DOCTYPE html><html><body style="font-family:system-ui,sans-serif;background:#f9f9f9;padding:20px">
    <div style="max-width:700px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.12)">
      <div style="background:#1e3a5f;color:#fff;padding:16px 20px">
        <h2 style="margin:0;font-size:18px">NFL Line Movement Alert</h2>
        <p style="margin:4px 0 0;font-size:13px;opacity:.8">${ts} · ${alerts.length} moves · ${highCount} HIGH · ${mediumCount} MEDIUM</p>
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:13px">
        <thead><tr style="background:#f1f5f9;color:#555;font-size:11px;text-transform:uppercase">
      ...
      <div style="padding:12px 20px;background:#f8fafc;font-size:11px;color:#999">
        Thresholds: Spread/Total HIGH>=1.5 MEDIUM>=0.5 · Moneyline HIGH>=15 MEDIUM>=5 · Lookback: ${LOOKBACK_MIN} min
      </div>
    </div></body></html>`;
...
async function sendEmail(subject, html, text) {
  if (!GMAIL_ADDR || !GMAIL_PASS) throw new Error('GMAIL_ADDRESS and GMAIL_APP_PASSWORD required');
...
async function run() {
  ...
  try {
    ...
    if (alerts.length === 0) { console.log('No significant moves -- nothing to send.'); return; }
