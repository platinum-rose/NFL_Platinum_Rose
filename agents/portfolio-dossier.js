...
//   • aggregated expert / podcast / article lean (consensus vs contrarian)
//
// Output:
//   • .nfl/portfolio/dossier-<date>.json   { meta, synthesis_input, detail }
...
function buildOddsView(snaps) {
  // group: market_type -> team -> book -> [rows asc by time]
  const g = {};
  ...
  for (const [mk, teams] of Object.entries(g)) {
    ...
    const teamOut = {};
    // Per-book latest implied for the market (for vig normalization on multiway).
    const bookOverround = {}; // book -> sum(latest implied) across teams
    if (MULTIWAY.has(mk)) {
    ...
    for (const [tm, books] of Object.entries(teams)) {
      const perBook = {};
      const fairProbs = [];
      let bestPrice = null, bestBook = null;   // outrights: best payout for a backer
      const winsLines = [];
      for (const [bk, rows] of Object.entries(books)) {
        ...
        if (isWins) {
          ...
          const fair = (MULTIWAY.has(mk) && or) ? ipRaw / or : ipRaw; // vig-stripped for multiway
      ...
      if (isWins) {
        ...
        // edge for a backer: vig-stripped fair prob minus best-price implied prob (positive = value)
...
// Cross-sport guard: NFL cities collide with NBA/other teams (Detroit Pistons →
// Lions via 'detroit'), so reject a resolution when the source text screams non-NFL.
...
function resolveNflTeam(str, contextText) {
  ...
  return normalizeTeam(str) || null; // canonical nickname (e.g. "Rams") or null
}

// ── lean aggregation: ONLY signals that cleanly resolve to an NFL team ────────
// Honesty patch (#2): loose headline-substring matching is gone — every lean is
// keyed to a canonical team via resolveNflTeam. Direction is real (back/fade for
// outrights, over/under for win totals). Direction-less free-text podcast intel
// strings are counted but NOT turned into leans (that's the #1 extraction job).
function buildLeanView(pickSignals, userPicks, podcastRows) {
  const leans = {}; // canonicalTeamName -> { team, article, expert, podcast, back, fade, over, under, samples }
  ...
  const add = (team, bucket, dir, conf, note, who) => {
    ...
    else e.back++; // a pick/signal ON a team = back it
  ...
  for (const row of podcastRows) {
    ...
    cov.podcast_intel_unparsed += intel.length; // free-text strings — deferred to #1 extraction
  ...
  return { leans, coverage: cov };
}

// ── compact synthesis input: only what the model needs to reason ─────────────
function buildSynthesisInput(markets, leans) {
  const findLean = (team) => {
    const canon = normalizeTeam(team); // exact canonical-team join, no substring guessing
    if (!canon) return null;
    const e = leans[canon];
    if (!e) return null;
    return { article: e.article, expert: e.expert, podcast: e.podcast,
      back: e.back, fade: e.fade, over: e.over, under: e.under, samples: e.samples };
  };
  const out = {};
  for (const [mk, teams] of Object.entries(markets)) {
    ...
    for (const [tm, v] of Object.entries(teams)) {
      if (v.type === 'wins') {
        rows.push({ team: tm, consensus_line: v.consensus_line, line_spread: v.line_spread,
          over_prob_median: v.over_prob_median, books: v.per_book, lean: findLean(tm) });
      } else {
    ...
    // sort each market by strongest signal (value_gap or divergence) for the model's attention
...
function toMarkdown(meta, synth) {
  const L = [`# Portfolio Dossier — ${meta.generated_at}`, '',
    `Season ${meta.season} · ${meta.snapshot_count} snapshots · books: ${meta.books.join(', ')}`,
    `Intel kept: article ${meta.intel_coverage.article.kept}, expert ${meta.intel_coverage.expert.kept}, podcast-picks ${meta.intel_coverage.podcast_pick.kept} (${meta.intel_coverage.podcast_intel_unparsed} free-text intel strings deferred)`, ''];
...
(async () => {
  ...
  const { leans, coverage } = buildLeanView(pickSignals, userPicks, podcastRows);
  const synthesis_input = buildSynthesisInput(markets, leans);
  console.log(`   intel kept: article ${coverage.article.kept}/${coverage.article.kept + coverage.article.dropped}, expert ${coverage.expert.kept}/${coverage.expert.kept + coverage.expert.dropped}, podcast-picks ${coverage.podcast_pick.kept}/${coverage.podcast_pick.kept + coverage.podcast_pick.dropped}; ${coverage.podcast_intel_unparsed} free-text intel strings deferred to signal extraction`);
  ...
  const dossier = { meta, synthesis_input, detail: markets };
