...
import { canonicalFuturesMarketIdentity, isTeamEligibleForFuturesMarket } from '../src/lib/futuresMarketIdentity.js';
...
// Freshness tiers still gate selection, but line shopping within the
// current 72-hour tier selects the best payout. Stale-vs-stale comparisons
// remain recency-first; see src/lib/futuresQuoteSelection.js.
const isBetterOffer = isBetterFuturesOffer;
...
function canonicalizeSnapshots(snaps) {
  ...
  for (const r of snaps || []) {
    ...
    if (!isPlayerOrMultiSideMarket(mk)) {
      const team = normalizeTeam(r.team) || r.team;
      // Preserve malformed source rows in raw storage for audit, but never let
      // a team from the wrong conference/division contaminate a market pool.
      if (!isTeamEligibleForFuturesMarket(team, mk)) continue;
      out.push({ ...r, team, selection: normalizeTeam(r.selection) || r.selection || team, book });
      continue;
    }
    if (mk === 'superbowl_matchup') {
      const matchup = canonicalFuturesMarketIdentity(r);
      out.push(matchup
        ? { ...r, team: matchup.label, selection: matchup.label, matchup_key: matchup.key, book }
        : { ...r, book });
      continue;
    }
    out.push({ ...r, book });
